...
def state(count , review_ques , review_ques_id, user_ques , review_ques_dict , user_ques_dict, db):
    def _state(update, context):
        ...
        if(count != 0 ): 
            key = get_question_id(count-1, review_ques_id)[0]# get question id from db        
        ...
        print(count , ' ' ,len(review_ques_dict) ) 
        if(count == len(review_ques_dict)):
            ...
            nques = get_question(count , review_ques)
            reply_text = "{}".format(get_question(count, review_ques))
            update.message.reply_text(reply_text, reply_markup=keyboards(count , context,  review_ques, user_ques ,  review_ques_dict, user_ques_dict , nques, db ))
