...
def user_info(count_info, review_ques , user_ques , user_ques_id, review_ques_dict ,user_ques_dict, db):
    def _user_info(update, context):
        ...
        if(count_info != 0 ):     
            key = get_userinfo_question_id(count_info-1 , user_ques_id)[0] # get question id from db
            question = get_userinfo_question(count_info-1 , user_ques) # get question from db
            if(db.get_user_type(question) == 'location'):    # if the question type is location 
        ...
        if(count_info == len(user_ques_dict)):
            ...
            nques = get_userinfo_question(count_info , user_ques) # get question id from db
            reply_text = "{}".format(get_userinfo_question(count_info, user_ques))
            update.message.reply_text(reply_text, reply_markup=keyboards(count_info , context,  review_ques, user_ques ,  review_ques_dict, user_ques_dict, nques , db))
